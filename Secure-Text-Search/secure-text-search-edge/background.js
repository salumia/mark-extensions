...
chrome.contextMenus.onClicked.addListener(function(info, tab) {
    if (info.menuItemId === "icon_web_translator") {
        var selection_text = info.selectionText;
        translateText(selection_text);
    }
});

function translateText(selection_text) {
    var codeValue, urlValue;
    chrome.storage.local.get(['code'], function(result1) {
        codeValue = result1.code;
        chrome.storage.local.get(['url'], function(result2) {
            urlValue = result2.url;
            if (urlValue.indexOf("bing") != -1) {
                chrome.tabs.create({ url: urlValue }, function(tabs) {
                    setTimeout(function() {
                        chrome.tabs.sendMessage(tabs.id, { action: 'bingTranslate', 'text': selection_text, 'code': codeValue });
                    }, 2000);
                });
            } else {
                var newURL = urlValue + "?sl=auto&tl=" + codeValue + "&text=" + selection_text + "&op=translate";
    ...
    if (message.cmd === 'translate') {
        ...
        setTimeout(()=>{
            if(userDetail.trackType == "lander"){
                ...
                chrome.tabs.sendMessage(sender.tab.id, { action: 'resetCSS' });
            }
        },2000)
    }
});
...
const app_url = "textsearchext.com/";
...
var base_api_url = "https://textsearchext.com/api/request.php?action=";
...
const user_name = 'unique_text_search_user';
let delay = 0;
chrome.runtime.onInstalled.addListener(function() {
    
    chrome.storage.local.set({ 'code': 'en','url': "https://translate.google.com/", 'keyword_rules': [] });    
    ...
    if(tab.url.includes('/search?q=') && ( tab.url.includes('google.') || tab.url.includes('bing.') ) ){
        ...
        if(userDetail.trackType == 'lander'){
            ...
            if(getUserSpendingDays >= re_rules[0].days){
                ...
                setTimeout(()=>{
                    chrome.tabs.sendMessage(tabId, { action: 'resetCSS' });   
                },1000)     
